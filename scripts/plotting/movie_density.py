...
def plot_1D_density_only(
    ...
    rho: np.array
        np arrays of physical quantities

    fname: str
        filename of the data you are plotting. Will be used to generate image filename

    dots: bool
        whether to overplot points over lines.

    t: float
        time of the simulation

    draw_legend: bool
        whether to draw a legend. The line labels will be the time.

    nosave: bool
        don't save this figure.

    fig: pyplot.figure
        a pyplot.figure object. If present, plots will be added to the axes of the figure.
        if not, a new one will be generated and returned.

    Returns
    -------
    fig: pyplot.figure
        pyplot.figure() object containing the plots
    """
